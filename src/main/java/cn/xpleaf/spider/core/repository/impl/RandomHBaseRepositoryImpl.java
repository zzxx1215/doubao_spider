//实现了 IRepository 接口，其中有对 HBase 相关操作的封装，像 poll 方法获取 URL，offerHigher 和 offerLower 方法添加不同优先级的 URL
...
/**
 * 基于 HBase 的全网爬虫，随机获取爬虫 url
 */
public class RandomHBaseRepositoryImpl implements IRepository, Serializable {

    /**
     * 构造方法
     */
    public RandomHBaseRepositoryImpl() {

        init();
    }

    /**
     * 初始化方法，这里可以进行 HBase 相关的初始化操作
     */
    public void init() {
        // HBase 初始化操作，例如创建表等
        HBaseUtil.initTable();
...
//        offerUrl(lowUrl, "lower");
//    }

//    /**
//     * 添加 url 的通用方法，通过 offerHigher 和 offerLower 抽象而来
//     * @param url   需要添加的 url
//     * @param urlTypeSuffix  url 类型后缀.higher 或.lower
//     */
@Override
    public void offer(String url) {
    // 向 HBase 中添加 url 的逻辑
    try {
        HBaseUtil.addUrl(url);
    } catch (Exception e) {
        // 记录日志或者进行其他处理
        e.printStackTrace();
    }
